            ...
            do
            {
                ...
                else
                {
                    if (((SqlClient.SqlConnection)owningConnection).ForceNewConnection)
                    {
                        ...
                        Console.WriteLine($"{DateTime.UtcNow} SqlClient | Force ReplaceConnection from Pool {connectionPool.PoolGroup.PoolKey.GetHashCode()} | Pool Count: {connectionPool.Count}");
                        connection = connectionPool.ReplaceConnection(owningConnection, userOptions, oldConnection);
                    }
                    else
                    {
                        Console.WriteLine($"{DateTime.UtcNow} SqlClient | TryGetConnection from Pool {connectionPool.PoolGroup.PoolKey.GetHashCode()} | Pool Count: {connectionPool.Count}");
